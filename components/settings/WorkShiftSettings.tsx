  ...
  return (
    <div className="min-w-0">
      <p className="text-sm text-slate-500">
        {currentShift
          ? t('Current week: {shift}. Shifts alternate automatically from the base week.', { shift: formatWorkShift(currentShift, language) })
          : t('Choose a base week and shift to start the alternating schedule.')}
      </p>
      ...
      <div className="mt-4 border-t border-slate-100 pt-3">
        <label className="block text-xs font-bold uppercase text-slate-500" htmlFor="transition-highlight">
          {t('Highlight the 1 → 2 shift break')}
        </label>
        <select
          id="transition-highlight"
          value={settings.transitionHighlight}
        ...
        >
          ...
          <option value="extended">{t('Friday through Monday')}</option>
        </select>
        <p className="mt-1 text-xs text-slate-400">
          {t('A subtle tint in the event calendar marks the long break before the second shift.')}
        </p>
      </div>

      <div className="mt-4 border-t border-slate-100 pt-3">
        <div className="text-xs font-bold uppercase text-slate-500">{t('Exceptions')}</div>
            ...
            {Object.entries(settings.overrides)
              ...
              .map(([week, shift]) => (
                <div key={week} className="flex items-center justify-between rounded bg-slate-50 px-2 py-1.5 text-sm">
                  <span className="text-slate-600">{week} · {formatWorkShift(shift, language)}</span>
                  <button
                    type="button"
                    onClick={() => removeOverride(week)}
                    className="rounded p-1 text-slate-400 hover:bg-red-50 hover:text-red-600"
                    title={t('Remove exception')}
                    aria-label={t('Remove {week} exception', { week })}
                  >
